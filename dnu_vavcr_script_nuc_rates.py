...
def calc_v_avcr(profile_number):
    ...
    return v_avcr
# ---------------------------------------------------------------------- #    
teff_obs = 5580; teff_obs_unc = 80 
logg_obs = 4.010; logg_obs_unc = 0.01
mass_obs = 1.11; mass_obs_unc = 0.06
lum_obs = 2.6; lum_obs_unc = 0.1
...
def plot_hr(profile_number, ax_i, ax_k):          
    ...
    ax[ax_i, ax_k].text(5550, 1.3, r'$f_{ov}/f_0$ = '+str(f_ov), fontsize=12)
...
def surf_correction(profile_number):
    ...
    weights = [0.9, 0.1]
    dof = [3, 44]
    chi2_spec  = (1/dof[0])*weights[0]*(chi_squared(lum_obs, lum, lum_obs_unc) + chi_squared(teff_obs, Teff, teff_obs_unc) + chi_squared(logg_obs, logg, logg_obs_unc))
    chi2_seism = (1/dof[1])*weights[1]*sum(chi_squared(corrected_frequency_table['obs_freqs'], corrected_frequency_table['corrected_freqs'], corrected_frequency_table['obs_errors']))
    chi2_total = chi2_spec + chi2_seism
    
    return corrected_frequency_table, chi2_total, chi2_spec, chi2_seism, a0, a3, v_ac
# ----------------------------- ECHELLE DIAGRAM ---------------------------------- #
def plot_echelle(ax_i, ax_k):
    symbols = ('o', '^', 's', 'D')
    muHer_obs = pd.read_csv('../updated_muHer_freqs.txt',
                        skiprows=2, sep='\s+')
    model = get_frequencies(best_profile)
    for ell in (0, 1, 2, 3):
...
Yini = row['Y']; Zini = row['Z']; alpha_mlt = row['alpha_mlt']; M = 1.13 
for M in (1.05, 1.105, 1.115, 1.125, 1.135, 1.145, 1.155, 1.165, 1.175, 1.185, 1.195):#(0.028, 0.0284, 0.0288, 0.0292, 0.0296, 0.030, 0.0304, 0.0308, 0.0312, 0.0316, 0.032, 0.0324, 0.0328, 0.0332, 0.0336, 0.034):#(0.027, 0.272, 0.274, 0.276, 0.278, 0.280, 0.282, 0.284, 0.286, 0.288, 0.290, 0.292, 0.294, 0.296, 0.298, 0.300, 0.302, 0.304, 0.306, 0.308, 0.31):#:#(1.78, 1.8, 1.82, 1.84, 1.86, 1.88, 1.9, 1.92, 1.94, 1.96, 1.98, 2.0, 2.02, 2.04, 2.06, 2.08, 2.1, 2.12):#:#
    for Yini in (0.280, 0.282, 0.284, 0.286, 0.288, 0.29, 0.292, 0.294, 0.296, 0.298, 0.300, 0.302, 0.304, 0.306, 0.308, 0.310):#(0.8, 0.82, 0.84, 0.86, 0.88, 0.9, 0.92, 0.94, 0.96, 0.98, 1.0, 1.02, 1.04, 1.06, 1.08, 1.1, 1.12, 1.14, 1.16, 1.18, 1.20):#, 1.20):#
        
        f_ov = 0.0
        rate = 1.0
        
        LOGS_directory = 'Y'+str(Yini)+'_Z'+str(Zini)+'_amlt'+str(alpha_mlt)+'_fov'+str(f_ov)+'_M'+str(M)+'_rate'+str(rate)
        print('Now running model:', LOGS_directory)
        ...
        subprocess.call('bash mesa-source.sh '+str(Yini)+' '+str(Zini)+' '+str(Zini)+' '+str(alpha_mlt)+' '+str(M)+' '+str(LOGS_directory)+' '+str(rate), shell=True)
        #subprocess.call('bash mesa-source.sh '+str(Yini)+' '+str(Zini)+' '+str(Zini)+' '+str(alpha_mlt)+' '+str(M)+' '+str(LOGS_directory)+' '+str(rate)+' '+str(f_ov), shell=True)
        ...
        while profile_number_check == None:
            ...
            if min_center_h1 < 0.0001:
                try:  
                    ...
                    profile_number = len(index_file.profile_number)+2     
                    print('now past MS')             
                except:
                    print('index messed up')
                    time.sleep(5)
                    continue
            elif min_center_h1 > 0.0001:
                print(min_center_h1)
                print('still on MS')
                time.sleep(20)
        ...
        if best_profile_number != None:
            ...
            if (min(v_avcr_diffs) != 9999999) and (len(v_avcr_diffs) > 0):         
                ...
                print('Now calculating all frequencies for best profile:', best_profile)
                ...
                corrected_frequency_table, chi2_total, chi2_spec, chi2_seism, a0, a3, v_ac = surf_correction(best_profile)
                corrected_frequency_table.write('~/Speciale/nuclear_reaction_rates/freqs_tables/freqs_'+str(LOGS_directory)+'.ascii', overwrite=True, format='ascii.fixed_width')
                mixed_modes_mask = (corrected_frequency_table['l'] == 1) & (corrected_frequency_table['obs_freqs'] < 905)  
                mixed_modes_table = corrected_frequency_table[mixed_modes_mask]  
                chi2_mixed_modes = 0.025*sum(chi_squared(mixed_modes_table['obs_freqs'], mixed_modes_table['corrected_freqs'], mixed_modes_table['obs_errors']))
                
                fig, ax = plt.subplots(2, 2, figsize=(18,12), dpi = 120)
                fig.suptitle(r'Best model: $\chi^2$ = '+str(round(chi2_total, 2)), fontsize=22, fontweight='bold')
                plot_hr(best_profile, 0, 0)
                plot_echelle(0, 1)
                plot_a0_evolution(1, 0)
                plot_oc(1, 1)
                fig.savefig('../figures/'+str(LOGS_directory)+'.png')
                
                Teff, logg, lum, age, radius = get_table_params(best_profile)
                
                base_table = Table.read('../parameters.ascii', format='ascii.fixed_width')
                table = Table()
                table['M'] = [M]
                table['Y'] = [Yini]
                table['Z'] = [Zini]
                ...
                table['chi2_mm'] = [chi2_mixed_modes] 
                
                combined_table = vstack([base_table, table])
                combined_table.write('../parameters.ascii', overwrite=True, format='ascii.fixed_width')
                
                os.chdir(mother_dir)
                
                print('figures succesfully saved and table updated --> moving on!')
                continue
